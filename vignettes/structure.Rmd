...
{
  use("tidywigits", c("Purple", "Wigits"))
  use("nemo")
  use("dplyr")
  use("glue", "glue")
  use("purrr", "map")
...
tool <- "purple"
toolu <- toupper(tool)
conf <- Config$new(tool, pkg = "tidywigits")
conf
```
...
```{r nemofy1}
ppl3 <- tidywigits::Purple$new(path = ppl_path)
...
For example, the {tidywigits} R package contains a `Wigits` class as a `Workflow` child based on a suite of
its own `Tool`s. Similarly to `Tool`, a `Workflow` object
contains functions such as `filter_files`, `list_files`, `tidy`, `write` and
`nemofy`:

```{r wigits_init}
w <- system.file("extdata/oa/purple", package = "tidywigits") |>
  tidywigits::Wigits$new()
